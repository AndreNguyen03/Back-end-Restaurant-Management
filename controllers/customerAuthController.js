...
const customerRegister = async (req, res) => {
    ...
    try {
        ...
        if (existingAccount) {
            return res.status(400).json({ success: false, message: "Tên tài khoản đã được sử dụng" });
        }
        ...
        // Create new account
        const newAccount = new accountModel({ username, password, role: "customer", userId: newCustomer._id });
...
const getOTP = async (req, res) => {
    ...
    const otpRequests = await Otp.find({ email }).sort({ createdAt: -1 }).limit(5);
    ...
    const currentTime = new Date();
    if (lastRequestTime && (currentTime - lastRequestTime) < 60000) { // 1 minute cooldown
        return res.status(429).json({
            success: false,
...
export { customerRegister, customerLogin, checkAuth, logout, getOTP, changePassword, getProfile, updateProfile  };